...
pagenum_t make_node(tableid_t table_id, pagenum_t parent_page_idx) {
    ...
    allocatedpage_t *page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, page_idx));

    page->page_header.is_leaf_page = 0;
    page->page_header.parent_page_idx = parent_page_idx;
    page->page_header.key_num = 0;

    buffered_write_page(table_id, page_idx);
...
pagenum_t create_tree(tableid_t table_id, recordkey_t key, const char* value,
    ...
    headerpage_t *header_page = reinterpret_cast<headerpage_t*>(buffered_read_page(table_id, 0));
    ...
    buffered_write_page(table_id, leaf_page_idx);
    buffered_write_page(table_id, 0);

    return leaf_page_idx;
}

pagenum_t find_leaf(tableid_t table_id, recordkey_t key) {
    headerpage_t *header_page = reinterpret_cast<headerpage_t*>(buffered_read_page(table_id, 0, false));
    ...
    internalpage_t *current_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, header_page->root_page_idx, false));
    while (!current_page->page_header.is_leaf_page) {
        ...
        current_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, current_page_idx, false));
    }
...
bool find_by_key(tableid_t table_id, recordkey_t key, char* value,
    ...
    if (trx_id &&
        ...
        return false;
    leafpage_t *leaf_page = reinterpret_cast<leafpage_t*>(buffered_read_page(table_id, leaf_page_idx, false));
...
pagenum_t insert_into_new_root(tableid_t table_id, pagenum_t left_page_idx,
    ...
    headerpage_t *header_page = reinterpret_cast<headerpage_t*>(buffered_read_page(table_id, 0));
    allocatedpage_t *left_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, left_page_idx));
    allocatedpage_t *right_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, right_page_idx));

    pagenum_t new_root_page_idx = make_node(table_id);
    internalpage_t *new_root_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, new_root_page_idx));
    ...
    *page_helper::get_leftmost_child_idx(new_root_page) = left_page_idx;
    ...
    buffered_write_page(table_id, new_root_page_idx);
    buffered_write_page(table_id, left_page_idx);
    buffered_write_page(table_id, right_page_idx);
...
pagenum_t insert_into_node(tableid_t table_id, pagenum_t parent_page_idx,
    ...
    int insert_position;
    internalpage_t *parent_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, parent_page_idx));
    ...
    buffered_write_page(table_id, parent_page_idx);
...
pagenum_t insert_into_node_after_splitting(tableid_t table_id,
    ...
    internalpage_t *page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, page_idx));
    ...
    internalpage_t *new_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, new_page_idx));
    ...
    *page_helper::get_leftmost_child_idx(new_page) =
    ...
    allocatedpage_t *leftmost_child_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, temp_branches[124].page_idx));
    ...
    for (int i = 125; i < 249; i++) {
        ...
        allocatedpage_t *child_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, temp_branches[i].page_idx));

        child_page->page_header.parent_page_idx = new_page_idx;

        buffered_write_page(table_id, temp_branches[i].page_idx);
    }
    ...
    buffered_write_page(table_id, page_idx);
    buffered_write_page(table_id, new_page_idx);
...
pagenum_t insert_into_parent(tableid_t table_id, pagenum_t left_page_idx,
                             recordkey_t key, pagenum_t right_page_idx) {
    allocatedpage_t *left_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, left_page_idx, false));
    ...
    internalpage_t *parent_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, parent_page_idx, false));
...
pagenum_t insert_into_leaf_after_splitting(tableid_t table_id,
    ...
    leafpage_t *new_leaf_page = reinterpret_cast<leafpage_t*>(buffered_read_page(table_id, new_leaf_page_idx));
    ...
    *leaf_sibling_idx = new_leaf_page_idx;
    ...
    buffered_write_page(table_id, leaf_page_idx);
    buffered_write_page(table_id, new_leaf_page_idx);
...
pagenum_t adjust_root(tableid_t table_id) {
    headerpage_t *header_page = reinterpret_cast<headerpage_t*>(buffered_read_page(table_id, 0, false));
    allocatedpage_t *root_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, header_page->root_page_idx, false));
    ...
    if (!root_page->page_header.is_leaf_page) {
        ...
        allocatedpage_t *new_root_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, header_page->root_page_idx));
...
pagenum_t coalesce_internal_nodes(tableid_t table_id, pagenum_t left_page_idx,
    ...
    recordkey_t old_key;
    internalpage_t *left_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, left_page_idx));
    internalpage_t *right_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, right_page_idx, false));
    internalpage_t *parent_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, left_page->page_header.parent_page_idx));


    page_helper::add_internal_key(left_page, seperate_key, *page_helper::get_leftmost_child_idx(right_page));

    allocatedpage_t *child_page = reinterpret_cast<allocatedpage_t*>(
        buffered_read_page(table_id, *page_helper::get_leftmost_child_idx(right_page)));
    child_page->page_header.parent_page_idx = left_page_idx;
    buffered_write_page(table_id, *page_helper::get_leftmost_child_idx(right_page));
    ...
    for (int i = 0; i < right_page->page_header.key_num; i++) {
        ...
        allocatedpage_t *child_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(table_id, right_page->page_branches[i].page_idx));
        child_page->page_header.parent_page_idx = left_page_idx;
...
pagenum_t coalesce_leaf_nodes(tableid_t table_id, pagenum_t left_page_idx,
    ...
    recordkey_t old_key;
    leafpage_t *left_page = reinterpret_cast<leafpage_t*>(buffered_read_page(table_id, left_page_idx));
    leafpage_t *right_page = reinterpret_cast<leafpage_t*>(buffered_read_page(table_id, right_page_idx, false));
    internalpage_t *parent_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, left_page->page_header.parent_page_idx, false));
    ...
    PageSlot* right_slot = page_helper::get_page_slot(right_page);
...
pagenum_t delete_internal_key(tableid_t table_id, pagenum_t internal_page_idx,
    ...
    internalpage_t *internal_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, internal_page_idx));
    
    page_helper::remove_internal_key(internal_page, key);
    buffered_write_page(table_id, internal_page_idx);

    pagenum_t parent_page_idx = internal_page->page_header.parent_page_idx;
    ...
    internalpage_t *sibling_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, sibling_page_idx));
    ...
    if (internal_page->page_header.key_num + sibling_page->page_header.key_num <
        ...
        if (!left_sibling) {
            ...
            leftmost_child_page = reinterpret_cast<allocatedpage_t*>(buffered_read_page(
        ...
        buffered_write_page(table_id, internal_page_idx);
        buffered_write_page(table_id, sibling_page_idx);
        buffered_write_page(table_id, parent_page_idx);
    }
...
pagenum_t delete_leaf_key(tableid_t table_id, pagenum_t leaf_page_idx,
    ...
    pagenum_t parent_page_idx = leaf_page->page_header.parent_page_idx;
    internalpage_t *parent_page = reinterpret_cast<internalpage_t*>(buffered_read_page(table_id, parent_page_idx));
    ...
    leafpage_t *sibling_page = reinterpret_cast<leafpage_t*>(buffered_read_page(table_id, sibling_page_idx));

    if (sibling_page->page_header.parent_page_idx !=
        leaf_page->page_header.parent_page_idx) {
        pagenum_t old_sibling_idx = sibling_page_idx;
        ...
        left_sibling = true;
        buffered_release_page(table_id, old_sibling_idx);
        sibling_page = reinterpret_cast<leafpage_t*>(buffered_read_page(table_id, sibling_page_idx));
    }
